...
import { addDays, addMonths, addYears, setHours, setMinutes, setSeconds } from 'date-fns';

@Injectable()
export class TasksService {
    constructor(private prisma: PrismaService) { }

    async create(userId: string, taskListId: string, createTaskDto: CreateTaskDto) {
                ...
                recurrenceRule: createTaskDto.isRecurring
                    ? {
                        create: {
                            ...
                            ordinalWeekdays: JSON.stringify(createTaskDto.ordinalWeekdays || []),
                        },
                    }
                    : undefined,
            },
                    ...
                    ? {
                        upsert: {
                            create: {
                                ...
                                ordinalWeekdays: JSON.stringify(updateTaskDto.ordinalWeekdays || []),
                            },
                            update: {
                                frequency: updateTaskDto.frequency || Frequency.DAILY,
                                interval: updateTaskDto.interval || 1,
            ...
            switch (task.recurrenceRule.frequency) {
                ...
                case Frequency.WEEKLY:
                    if (
                        ...
                        Math.floor(currentDate.getDate() / 7) % task.recurrenceRule.interval === 0
                    ) {
                        shouldCreateOccurrence = true;
                    }
                    currentDate = addDays(currentDate, 1);
                ...
                case Frequency.YEARLY:
                    if (
                        task.recurrenceRule.monthsOfYear.includes(currentDate.getMonth() + 1) &&
